...
int main(void)
{
  ...
  MX_TIM1_Init();
  /* USER CODE BEGIN 2 */
	__HAL_TIM_CLEAR_IT(&htim1, TIM_IT_UPDATE); //TIM1->SR = ~TIM_SR_UIF;
  ...
  {
	enc_val = TIM1->CNT;
	if (button_state && last_enc != enc_val) {
		button_state = 0;
	}

	if (enc_val >= 0 && enc_val <= 6) {
